...
struct DirectionMapViewController: UIViewRepresentable {
    ...
    func makeUIView(context: Self.Context) -> GMSMapView {
        ...
        pick.title = "Start Location"

        pick.iconView = self.icon("mappin.and.ellipse", .red)
        //pick.icon = UIImage(named: "mappin.and.ellipse")
        
        let drop = GMSMarker(position: stateHandler.userDropLocation!)
        drop.title = "End Location"
        
        drop.iconView = self.icon("mappin.and.ellipse", .red)
       // pick.icon = UIImage(named: "mappin.and.ellipse")
